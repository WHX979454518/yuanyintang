...
* 后面产品有这需求再加也很方便
* */
class PondBigTxtTabLayout : RecyclerView {
    ...
    internal inner class Adapter(data: List<OrderTypeBean.DataBean.WhereBean>) : BaseQuickAdapter<OrderTypeBean.DataBean.WhereBean, BaseViewHolder>(R.layout.text_pond_item, data) {
...
//                    ckTitle.setTextColor(Color.parseColor(unCkedColor))
                    ckTitle.setTextColor(Color.argb(77, 255, 255, 255));   //文字透明度
//                    ckTitle.setTextColor(Color.parseColor("#1a1a1a"))
                } else {
//                    ckTitle.setTextColor(Color.parseColor("#ffffff"))
                    ckTitle.setTextColor(Color.argb(77, 255, 255, 255));   //文字透明度
                }
