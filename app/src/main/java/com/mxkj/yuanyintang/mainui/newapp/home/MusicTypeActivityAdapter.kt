...
class MusicTypeActivityAdapter(data: List<MusicClassification.DataBean.DataListBean>, private val fragmentManager: FragmentManager?, private val biaoshi: String?, private val page: Int, private val id: String?, var isEdit: Boolean) : BaseQuickAdapter<MusicClassification.DataBean.DataListBean, BaseViewHolder>(R.layout.musicrecycle_item_music_list, data) {
    //    musictypeactivity_item_music_list
    private var checkedSongListener: ClickCheckedSongListener? = null
    override fun convert(helper: BaseViewHolder, item: MusicClassification.DataBean.DataListBean, position: Int) {
        ...
        when {
            item.imgpic_info != null ->
                Glide.with(mContext)
                        .load(item.imgpic_info.link)
                        .into(helper.getView(R.id.iv_music));
//                ImageLoader.with(mContext).getSize(200, 200).setUrl(item.imgpic_info.link).into(helper.getView(R.id.iv_music))
        }
        ...
        when {
            item.isLocalMusic!! -> helper.getView<View>(R.id.iv_more).visibility = View.GONE
        ...
        RxView.clicks(helper.getView(R.id.jinrituijian_img))
                ...
                .subscribe {
                    ...
                    if (biaoshi== "musicclassification"){
                        Log.e("item",""+item.id)
                        Log.e("item",""+item.mv_info.link)
                        Log.e("item",""+item.uid)
                        Log.e("item",""+item.title)
                        Log.e("item",""+item.nickname)
                        Log.e("item",""+item.imgpic_link)
                        if(null==item.mv_info.link||item.mv_info.link.equals("")){
