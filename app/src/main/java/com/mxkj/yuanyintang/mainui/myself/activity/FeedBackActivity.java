...
import com.mxkj.yuanyintang.mainui.myself.apdater.FeedBackAdapter;
...
public class FeedBackActivity extends StandardUiActivity implements BGASortableNinePhotoLayout.Delegate, EasyPermissions.PermissionCallbacks {
...
//            if (selectedList.size() > 0) {
                ...
                IMGCompressUtils.CompressorImage1(selectedList.size(), getApplicationContext(), selectedList, new IMGCompressUtils.CompressCallback() {
                    ...
                    public void fileCallback(@android.support.annotation.NonNull List<File> files) {
                        if (files != null && files.size() > 0) {
                            ...
                            fileUploadUtil.upload(FeedBackActivity.this, 1, new FileUploadUtil.UpLoadCallback() {
                                @Override
    ...
    private void publish(String s) {
        NetWork.INSTANCE.postFeedback(this, "", et_qq.getText().toString(), et_content.getText().toString(), s, "", pid, new NetWork.TokenCallBack() {
            @Override
            public void doNext(String resultData, Headers headers) {

                BaseConfirmDialog.Companion.newInstance().confirmText("好的").cancleText("").isOneBtn(true).title("提交成功").isShowYxy(true).content("感谢您的反馈，我们会尽快处理哒~").showDialog(FeedBackActivity.this, new BaseConfirmDialog.onBtClick() {
                    @Override
    ...
    public void onPermissionsDenied(int requestCode, List<String> perms) {
        if (requestCode == REQUEST_CODE_PERMISSION_PHOTO_PICKER) {
            Toast.makeText(FeedBackActivity.this, "您拒绝了选取照片所需的权限!", Toast.LENGTH_SHORT);
        }
    }
