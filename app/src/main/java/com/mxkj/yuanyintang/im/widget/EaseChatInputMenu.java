...
import com.mxkj.yuanyintang.im.domain.EaseEmojiconGroupEntity;
import com.mxkj.yuanyintang.im.domain.EmojiconExampleGroupData;
import com.mxkj.yuanyintang.im.utils.EaseSmileUtils;
import com.mxkj.yuanyintang.im.widget.emojicon.EaseEmojiconMenu;
import com.mxkj.yuanyintang.im.widget.emojicon.EaseEmojiconMenuBase;
...
public class EaseChatInputMenu extends LinearLayout {
    ...
    private EaseChatSpeakMenu speak_container;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        speak_container = (EaseChatSpeakMenu) findViewById(R.id.speak_container);

        // extend menu
        chatExtendMenu = (EaseChatExtendMenu) findViewById(R.id.extend_menu);

        speak_container.setSendSpeakOnClickListener(new EaseChatSpeakMenu.SendSpeakOnClickListener() {
            @Override
            public void sendVoiceMsg(String fileName, int length) {
                if (null != listener) {
                    listener.onSendVoiceMsg(fileName, length);
                }
            }
        });
    }
...
//                emojiconGroupList.add(new EaseEmojiconGroupEntity(R.drawable.ee_1,  Arrays.asList(EaseDefaultEmojiconDatas.getData())));
        ...
        if (chatExtendMenuContainer.getVisibility() == View.GONE) {
            ...
            handler.postDelayed(new Runnable() {
                public void run() {
                    ...
                    speak_container.setVisibility(View.GONE);
                }
            }, 50);
        } else {
        ...
        if (chatExtendMenuContainer.getVisibility() == View.GONE) {
            ...
            handler.postDelayed(new Runnable() {
                public void run() {
                    ...
                    chatExtendMenu.setVisibility(View.GONE);
                    emojiconMenu.setVisibility(View.GONE);
                    speak_container.setVisibility(View.VISIBLE);
                }
            }, 50);
        } else {
            if (speak_container.getVisibility() == View.VISIBLE) {
                chatExtendMenuContainer.setVisibility(View.GONE);
                speak_container.setVisibility(View.GONE);
            } else {
                chatExtendMenu.setVisibility(View.GONE);
    ...
    public interface ChatInputMenuListener {
        ...
        void onSendVoiceMsg(String fileName, int duration);
    }
