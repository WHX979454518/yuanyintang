...
import com.mxkj.yuanyintang.im.adapter.EaseMessageAdapter;
...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void setUpBaseView() {
        ...
        Log.d(TAG, "setUpBaseView: ----自己--->" + message.getStringAttribute(EaseConstant.EXTRA_USER_AVATAR, ""));
        Log.d(TAG, "setUpBaseView: -----他的-->" + message.getStringAttribute(EaseConstant.EXTRA_OTHER_AVATAR, ""));
        Log.d(TAG, "setUpBaseView: ------->" + (message.direct() == Direct.SEND));
        if (null != userAvatarView) {
            Glide.with(context).load(message.getStringAttribute(EaseConstant.EXTRA_USER_AVATAR, "") + "/" + dip2px(50) + "/" + dip2px(50)).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.drawable.ease_default_avatar).into(userAvatarView);
            //set nickname and avatar
        }
        if (null != usernickView) {
            String otherNickname = null;
            otherNickname = message.getStringAttribute(EaseConstant.EXTRA_USER_NAME, "");
            usernickView.setText(otherNickname);
        }
