...
public class BGASortableNinePhotoLayout extends NoScrollRecyclerView implements BGAOnItemChildClickListener, BGAOnRVItemClickListener {
    ...
    public BGASortableNinePhotoLayout(@NonNull Context context, AttributeSet attrs) {
        super(context, attrs);
        initDefaultAttrs();
    ...
    private void initDefaultAttrs() {
        ...
        mSortable = true;
        mEditable = true;
        mDeleteDrawableResId = R.drawable.icon_clear;
        ...
        //视图右侧的留白  为0的话视图宽度填满屏幕
//        mOtherWhiteSpacing = BGAPhotoPickerUtil.dp2px(60);
        mOtherWhiteSpacing = BGAPhotoPickerUtil.dp2px(0);
    }
    ...
    private void initCustomAttr(int attr, @NonNull TypedArray typedArray) {
     ...
     * @param photos
     */
    public void addMoreData(@Nullable ArrayList<String> photos) {
        if (photos != null) {
...
//            mImageSize = BGAPhotoPickerUtil.getScreenWidth() / (mItemSpanCount > 3 ? 8 : 6);
        ...
        @Override
        protected void setItemChildListener(@NonNull BGAViewHolderHelper helper, int viewType) {
        ...
        protected void fillData(@NonNull BGAViewHolderHelper helper, int position, String model) {
            ...
            if (isPlusItem(position)) {
                helper.setVisibility(R.id.iv_item_nine_photo_flag, GONE);
                helper.setImageResource(R.id.iv_item_nine_photo_photo, mPlusDrawableResId);
            } else {
                if (mEditable) {
                    helper.setVisibility(R.id.iv_item_nine_photo_flag, VISIBLE);
